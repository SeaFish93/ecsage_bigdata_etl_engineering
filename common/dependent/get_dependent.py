    ...
    if len(task_list) > 0:
       ...
       if len(get_list_depend_data) > 0:
         ...
         for difference_data in set(get_list_depends).difference(set(task_list)):
             ok, get_delete_sql = etl_md.execute_sql(sqlName="get_delete_depend_sql",Parameter={"task_id": target_db + "_" + target_table,"dep_task_id": difference_data}, IsReturnData="N")

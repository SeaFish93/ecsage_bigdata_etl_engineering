...
class BeelineNoSqlDB(BaseDB):
    def __init__(self, host=None, port=None, user=None, password=None, metastore_uris=None):
        ...
        self.conn = "/opt/soft/hive/apache-hive-2.1.1-bin/bin/beeline -u 'jdbc:hive2://%s:%s/default;auth=noSasl' -n %s -d org.apache.hive.jdbc.HiveDriver -p '%s'" % (self.metastore_uris, int(self.port),self.user, self.password)
        #print(self.conn,"#########################################=======================")

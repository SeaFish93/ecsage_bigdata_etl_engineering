...
#获取dag信息
ok, get_dags = etl_meta.execute_sql(sqlName="get_data_dags_sql",Parameter={"exec_type":"load"},IsReturnData="Y")
if ok is False:
...
for dag_info in get_dags:
    ...
    # 同步任务配置
    ok, get_tasks = etl_meta.execute_sql(sqlName="get_data_sync_tasks_sql",Parameter={"dag_id":dag_id},IsReturnData="Y")
    if ok is False:
    ...
    if len(get_tasks) > 0:
       for tasks_info in get_tasks:
          ...
          no_run_time = []
          if tasks_info[17] is not None and tasks_info[17] != "":
            for time_no in (tasks_info[17]).split(","):
              no_run_time.append(int(time_no))
          task = locals()
          #定义task对象
          if batch_type == "hour":
             # 动态创建dag实例
             task['%s' % (task_id)] = PythonOperator(task_id=task_id,
              ...
              task['%s' % (task_id)] = PythonOperator(task_id=task_id,
                                         python_callable=sync_mysql_main,
                                         provide_context=True,
       ...
       for task_name in tasks:
          if task_name["batch_type"] == "day":
              ...
              ok, task_upstream_deps = etl_meta.execute_sql(sqlName="get_upstream_depend_sql",Parameter={"dep_task_id": task_name["task_id"]}, IsReturnData="Y")
              print(task_upstream_deps,"===========================@@@@@@@@@@@@@@@@@@@@@@")

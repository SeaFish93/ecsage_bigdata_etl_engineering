...
from ecsage_bigdata_etl_engineering.bi_etl.sync.file.interface.tasks import get_not_page_tc_timing as get_not_page_celery
from ecsage_bigdata_etl_engineering.bi_etl.sync.file.interface.tasks import get_pages_tc_timing as get_pages_celery
...
tc_celery_works_hostnames = eval(conf.get("Interface", "tc_celery_works_hostname"))
...
def get_data_2_etl_mid(BeelineSession="",TargetDB="",TargetTable="",AirflowDag="",AirflowTask="",TaskInfo="",ExecDate="",ArrayFlag="",ExPartField=""):
  ...
  for tc_celery_works_hostname in tc_celery_works_hostnames:
      os.system("mkdir -p %s" % (local_dir.replace("ecsage_data", "ecsage_data_%s" % tc_celery_works_hostname)))
      os.system("""chmod -R 777 %s""" % (local_dir.replace("ecsage_data", "ecsage_data_%s" % tc_celery_works_hostname)))
  ...
  sql2 = '''
      ...
      from metadb.tc_service_account
  '''%(task_flag)
    ...
    if os.path.exists(local_dir.replace("ecsage_data", "ecsage_data_%s"%tc_celery_works_hostname)):
     ...
     for files in target_file:
         if str(data_task_file.split("/")[-1]).split(".")[0] in files and '.lock' not in files:
             data_task_file_list.append("%s/%s"%(local_dir.replace("ecsage_data", "ecsage_data_%s"%tc_celery_works_hostname), files))
  #数据落地至etl_mid
